...
def main():
    ...
    opt.image_shape = (opt.channels, opt.img_size, opt.img_size)
    ...
    class Encoder(nn.Module):
        def __init__(self):
            ...
            self.model = nn.Sequential(
                nn.Linear(int(np.prod(opt.image_shape)), 512),
                nn.LeakyReLU(0.2, inplace=True), nn.Linear(512, 512),
    ...
    class Decoder(nn.Module):
        def __init__(self):
            ...
            self.model = nn.Sequential(
                ...
                nn.LeakyReLU(0.2, inplace=True), nn.Linear(512, 512),
                nn.BatchNorm1d(512), nn.LeakyReLU(0.2, inplace=True),
                nn.Linear(512, int(np.prod(opt.image_shape))), nn.Tanh())

        def forward(self, z):
            img_flat = self.model(z)
            img = img_flat.view(img_flat.shape[0], *opt.image_shape)
            return img
    ...
    def sample_image(n_row, batches_done):
        ...
        # Sample noise
        z = Variable(
            Tensor(np.random.normal(0, 1, (n_row**2, opt.latent_dim))))
        gen_imgs = decoder(z)
        save_image(
            gen_imgs.data,
            'images/%08d.png' % batches_done,
            nrow=n_row,
            normalize=True)
